...
/* ===================================== declare our vectors ===================================== */
	...
	std::vector<std::vector<mfdata_v_t>> query_arr(2, std::vector<mfdata_v_t>(out_cnt));
...
/* remember - if it's suppsoed to be m_axi, no need to create a stream. just use the created vector, ie: foo_arr.data() */
	...
	hls::stream<mfdata_v_t> qs_out("Query output Stream");
	...
	matmult_kernel(qs_out, tq_sf_in, tq_in, qw_sf_in, qw_in, MODEL_ELEMENTS, MODEL_ELEMENTS);
	...
	parse_results<mfdata_v_t, float>(query_arr[0], query_arr[1]);
